...
ratioDicResp <- c(0.642, 0.856, 1, 1.07, 1.284, 1.498)

#line colors for each run in the loop
modelcolors <- c("darkorange3", "limegreen", "blue", "black", "magenta", "red")

#changing line widths
linewidths <- c(1, 1, 1, 2, 1, 1)

#loop simulation output
modelsims <- vector(mode = "list", length = length(ratioDicResp))

#startloop
for(index in 1:length(ratioDicResp)){
  ...
  #Setting up NLS parameters for onestation
  runmodelDO<-function(
    params,
    metabModel) 
  {
    ...
    if(!is.na(params["ratioDicResp"])) {
      metabModel$setMetabDoParam("RatioDicResp", params["ratioDicResp"])
    }
  ...
  modelDoDic <- CMetabLagrangeDoDic$new(
    ...
    parTotal = -1,
    ratioDicCFix = -0.32,
    ratioDicCResp = ratioDicResp[index],
...
#finding min and max of each run of model DoDIC and min and max of change across the reach
minmaxmatrix <- sapply(
  X = modelsims,
...
text(
  ...
  adj = c(1,0),
  labels = bquote(.("DIC:C Fixation = -0.32"))
)
...
#Plot change in pCO2 concentrations
plot(
  x = modelDoDic$downstreamTimePOSIX,
